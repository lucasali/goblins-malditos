...
function loadCollection() {
  ...
  if (savedCollection) {
    try {
      ...
      if (failedSeeds > 0) {
        showErrorMessage(`Não foi possível carregar ${failedSeeds} goblin(s) da coleção`)
      }
    } catch (error) {
      console.error('Erro ao carregar a coleção de goblins:', error)
      showErrorMessage('Erro ao carregar a coleção de goblins')
    }
  }
}
...
function addGoblinToCollection() {
  ...
  if (seeds.includes(newSeed.value)) {
    showErrorMessage('Este goblin já está na coleção')
    return
  }
  ...
  newSeed.value = ''
  showSuccessMessage('Goblin adicionado à coleção com sucesso!')
}
...
function removeGoblinFromCollection(seed: string | undefined) {
  ...
  showSuccessMessage('Goblin removido da coleção')
}

// Função para limpar toda a coleção
function clearCollection() {
  if (confirm('Tem certeza que deseja limpar toda a coleção?')) {
    goblinCollection.value = []
    localStorage.removeItem('goblinCollection')
    showSuccessMessage('Coleção limpa com sucesso')
  }
}
...
<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-goblin-green mb-6">Coleção de Goblins</h1>
    
    <!-- Mensagem de erro ou sucesso -->
    <div 
      v-if="showMessage" 
      class="mb-4 p-3 rounded-md transition-all duration-300"
      :class="isError ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-green-100 text-green-700 border border-green-300'"
    >
      {{ message }}
    </div>
    
    <!-- Formulário para adicionar goblin -->
    <div class="bg-goblin-dark p-4 rounded-lg mb-6">
      <h2 class="text-xl text-white mb-3">Adicionar Goblin à Coleção</h2>
    ...
    <div class="flex justify-between items-center mb-4">
      <div>
        <span class="text-goblin-brown">{{ goblinCollection.length }} goblins na coleção</span>
      </div>
    ...
    <!-- Mensagem quando não há goblins -->
    <div v-if="goblinCollection.length === 0" class="text-center py-10 bg-parchment rounded-lg">
      <p class="text-goblin-dark text-xl">Nenhum goblin na coleção</p>
      <p class="text-goblin-brown mt-2">Adicione goblins usando as seeds geradas na página inicial</p>
    </div>
